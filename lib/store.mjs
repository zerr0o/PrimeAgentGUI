...
function sessionMessages(entries, branch = selectedBranch(entries)) {
  ...
  for (const e of branch) {
    ...
    if (e.type === 'custom_message' && e.display) {
      messages.push({ id: e.id, role: 'system', text: textOf(e.content), tools: [], timestamp: e.timestamp });
    }
    ...
    if (m.role === 'user' || m.role === 'assistant') {
      ...
      messages.push({
        ...
        attachments: content
          ...
          .map((c) => ({ type: 'image', mimeType: c.mimeType })),
      });
      ...
      messages.push({ id: e.id, role: 'system', text: textOf(m.content), tools: [], timestamp: e.timestamp });
    }
  }
...
export function createStore({ sessionDir, dataDir, initialCwd }) {
  ...
  async function init() {
    ...
    if (!state.projects.length && initialCwd)
  ...
  async function overview() {
    ...
    const projects = new Map(state.projects.map((p) => [cwdKey(p.cwd), { ...p, sessions: [] }]));
  ...
  return { init, overview, history, project, patchSession, sessionDir };
}
