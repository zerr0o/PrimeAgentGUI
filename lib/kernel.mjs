...
function checkCode(skills) {
  return `import importlib, json, traceback
...
for check in checks:
    try:
        ...
        if check['name'] == 'rlm':
            ...
            assert callable(module.run), 'rlm.run must be callable'
...
export async function ensureLocalKernel(
  ...
  for (const message of discovery.diagnostics || []) onProgress(`Skills Python : ${message}`);
  ...
  async function validate(python, external = false) {
    ...
    try {
      ...
      if (!Array.isArray(result.failures)) throw new Error('Résultat de validation invalide.');
    } catch (error) {
      throw new Error(`Vérification du Python ${python} : ${error.message}`);
    }
    for (const failure of result.failures) {
      const essential = failure.kind === 'runtime' || failure.name === 'agent_message';
      const diagnostic = `Python ${python} — ${essential ? 'fonction essentielle' : 'skill optionnelle'} « ${failure.name} » : ${failure.error}`;
      if (!external || essential) throw new Error(diagnostic);
      onProgress(`Avertissement : ${diagnostic}`);
    }
  }
  ...
  if (!existsSync(join(source, 'pyproject.toml')))
    throw new Error('Le runtime Python fourni avec Prime Agent est introuvable. Réinstallez Prime Agent.');
  ...
  const job = (async () => {
    ...
    try {
      ...
      onProgress(`Installation du runtime et de ${skills.length} skills Python…`);
      await run(
        uv,
        [
          'pip',
      ...
      const contents =
        JSON.stringify(
          { schema: SCHEMA, identity, python, skills, inputs, preparedAt: new Date().toISOString() },
          null,
          2,
        ) + '\n';
      ...
      onProgress(`Noyau Python prêt : runtime et ${skills.length} skills vérifiés.`);
      return python;
    } finally {
      await release();
    }
