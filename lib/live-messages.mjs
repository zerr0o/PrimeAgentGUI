...
export function createLiveMessages({ getRuns, getClient, fileStore, validateMessage, now = Date.now }) {
  ...
  async function send(sessionId, body) {
    ...
    const message = messageText(
      (images.length || files.length) && body.message === '' ? 'Analyse les pièces jointes.' : body.message,
    );
    ...
    return once(`${sessionId}:${body.requestId}`, value, async () => {
      ...
      if (files.length && !fileStore) throw new HttpError(503, 'Stockage des fichiers indisponible.');
      const nativeMessage = appendFileMessage(message, files.length ? await fileStore.save(files) : []);
  ...
  async function mutate(sessionId, body) {
    ...
    if (mutation.type === 'replace') {
      ...
      if (
        ...
        throw new HttpError(
          400,
          'Supprimez puis renvoyez ce message pour le remplacer par une commande de session, ou inversement.',
        );
