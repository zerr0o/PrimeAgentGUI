...
export function fileLaunchMode(path) {
  ...
  throw new HttpError(400, 'Ce type de fichier ne peut pas être ouvert depuis le Studio.');
}
...
export function createFileOpener({
  ...
  return async (path) => {
    if (typeof path !== 'string' || !isAbsolute(path) || /[\x00-\x1f]/.test(path))
      throw new HttpError(400, 'Fichier invalide.');
    if (!(await stat(path).catch(() => null))?.isFile()) throw new HttpError(404, 'Fichier introuvable.');
    ...
    const job = (async () => {
      try {
        if (platform === 'win32') {
          const { stdout } = await run(
            ...
            ],
            {
              ...
              maxBuffer: 16384,
              encoding: 'utf8',
              env: { ...env, PRIME_STUDIO_OPEN_FILE: path, PRIME_STUDIO_FILE_MODE: mode },
            },
          );
        ...
        throw new HttpError(
          502,
          'Le PC n’a pas pu ouvrir ce fichier. Vérifiez qu’une application est associée à ce format.',
        );
      }
    })();
    pending.set(key, job);
