...
export function agentStatus(state = {}) {
  ...
  if (state.isSessionActive) return 'working';
  return 'idle';
}
...
export function createSessionInspector({ store, agentHome, sessionDir, getClient, getRun, readEdges }) {
  ...
  async function inspect(cwd, id) {
    ...
    if (!validId(id)) throw new HttpError(400, 'Session invalide.');
    const root = await store.history(id);
    if (cwdKey(root.cwd) !== cwdKey(project.cwd))
      throw new HttpError(404, 'Session introuvable dans ce projet.');
  ...
  async function build(root) {
    ...
    if (running) {
      ...
      try {
        ...
        notes.push('Le suivi en direct est momentanément indisponible.');
      } finally {
        client?.close?.();
      }
    }
    ...
    try {
      ...
      notes.push('L’historique des délégations n’est pas disponible.');
    }
    ...
    return {
      session: { id: root.id, status: rootStatus, usage: rootAgent.usage },
  ...
  return {
    ...
    async history(cwd, rootId, childId) {
      if (!validId(childId)) throw new HttpError(400, 'Agent invalide.');
      const result = await inspect(cwd, rootId),
        file = result.files?.get(childId);
      if (!file) throw new HttpError(404, 'Historique de cet agent indisponible.');
      const actual = await realpath(file).catch(() => null);
      if (!actual || cwdKey(actual) !== cwdKey(file))
        throw new HttpError(404, 'Historique déplacé. Actualisez les agents.');
      if (childId !== rootId) {
        const info = await stat(actual).catch(() => null);
        if (!info?.isFile() || info.size > 16 * 1024 * 1024)
          throw new HttpError(413, 'Historique trop volumineux.');
      }
      const history = await store.inspectFile(file);
      if (!history) throw new HttpError(404, 'Historique introuvable.');
      return {
