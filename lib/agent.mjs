...
export function agentEnvironment({ agentHome, sessionDir, env = process.env } = {}) {
  ...
  for (const name of Object.keys(result))
    if (
      ...
      ['PRIME_GUI_CONTROL', 'PRIME_GUI_CLI_ROOT'].includes(name)
    )
      delete result[name];
...
export function normalizeMessage(message = {}) {
  ...
  return {
    role: cleanString(message.role, 40),
      ...
      return [
        {
          kind: 'status',
          status: 'running',
          message: event.errorMessage ? diagnostic(event.errorMessage) : 'Contexte optimisé.',
        },
      ];
    case 'agent_start':
...
export function createAgentRuntime(options = {}) {
  ...
  async function getModels() {
        ...
        try {
          ...
          for (const provider of builtin.getProviders()) {
            if (configured.has(provider))
              for (const model of builtin.getModels(provider)) add(provider, model);
          }
        } catch {
    ...
    for (const [provider, value] of Object.entries(providers)) {
      for (const model of Array.isArray(value?.models) ? value.models : []) add(provider, model);
      if (record(value?.modelOverrides)) {
        for (const [modelId, model] of Object.entries(value.modelOverrides)) {
          const old = models.get(`${provider}/${modelId}`);
    ...
    for (const value of recent) {
      ...
      if (slash > 0 && slash < id.length - 1 && !models.has(id))
        add(id.slice(0, slash), { id: id.slice(slash + 1) });
    }
    ...
    if (defaultId.includes('/') && !models.has(defaultId)) {
      ...
      add(defaultId.slice(0, slash), { id: defaultId.slice(slash + 1) });
    }
    return {
  ...
  async function startInner(input) {
    if (closing) throw new Error('Le runtime est en cours d’arrêt.');
    if (!cli) throw new Error('Prime Agent introuvable. Définissez PRIME_AGENT_CLI.');
    if (!record(input)) throw new TypeError('Paramètres de session invalides.');
    if (
    ...
    if (typeof input.message !== 'string' || !input.message.trim()) throw new Error('Le message est vide.');
    if (Buffer.byteLength(input.message) > 512 * 1024) throw new Error('Le message dépasse 512 Ko.');
    ...
    if (
      ...
      throw new Error('Fichier de session introuvable.');
    await mkdir(sessionDir, { recursive: true });
    if (process.platform === 'win32' && cli.packageDir && !env.PRIME_AGENT_KERNEL_PYTHON) {
      env.PRIME_AGENT_KERNEL_PYTHON = await ensureLocalKernel({
    ...
    function consume(line) {
      ...
      if (
        ...
        failure = diagnostic(event.message.errorMessage) || 'Le fournisseur de modèle a renvoyé une erreur.';
    ...
    child.stdout.on('data', (chunk) => {
      ...
      if (buffer.length > MAX_LINE) {
        failure = 'Une réponse de Prime Agent dépasse la limite de lecture (16 Mo).';
  ...
  return {
    ...
    getLiveEndpoint() {
      return daemon && cli?.packageDir ? {
        packageDir: cli.packageDir, socketPath: daemon.socketPath,
        agentDir: agentHome, supervisorPid: daemon.pid,
      } : null;
    },
    async close() {
      closing = true;
