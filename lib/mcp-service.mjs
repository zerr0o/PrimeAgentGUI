...
import { localKernelPython } from './kernel.mjs';
...
export function createMcpService({
  ...
  python = environment.PRIME_AGENT_KERNEL_PYTHON || localKernelPython(),
  spawnProcess = spawn,
}) {
  const jobs = new Map(),
    probes = new Set();
  ...
  async function runProbe(body) {
    ...
    if (!existsSync(python))
      throw new HttpError(503, tr('server.le_moteur_python_manque_executez_npm_run_setup_runtime_sur_le_pc'));
