  ...
  async function launch(path) {
    try {
      if (platform === 'win32') {
        ...
        if (result.opened !== true || result.visible !== true) throw new Error('Explorer is not visible');
      } else {
      ...
      throw new HttpError(
        502,
        platform === 'win32'
          ? 'Impossible d’afficher ce dossier dans l’Explorateur du PC. Réessayez.'
          : 'Impossible d’ouvrir ce dossier sur le PC. Réessayez.',
      );
    }
  }
  return async (cwd) => {
