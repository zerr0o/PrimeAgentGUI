...
export function createRemoteUpdates({
  ...
  async function writeJsonAtomic(path, value) {
    ...
    if (target && target.isSymbolicLink()) throw new HttpError(500, tr('server.demande_de_mise_a_jour_invalide'));
  ...
  async function fetchPublished() {
    const at = now();
    if (published && at - published.fetchedAt < METADATA_TTL_MS) return published;
    let response;
    try {
      response = await fetchImpl(UPDATER_META_URL, { signal: AbortSignal.timeout(15000) });
    } catch {
  ...
  async function readHeartbeat(at) {
    ...
    return { alive: true, aliveAt: beat.aliveAt, appVersion: parseSemver(beat.appVersion) ? beat.appVersion : null };
  }
  ...
  async function metadata() {
    const at = now();
    const [pub, beat, req] = await Promise.all([fetchPublished(), readHeartbeat(at), publicRequest(at)]);
    return {
  ...
  async function requestUpdate(body) {
    ...
    if (!pub) throw new HttpError(503, metaError || tr('server.mise_a_jour_indisponible_verifiez_votre_connexion'));
