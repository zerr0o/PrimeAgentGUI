...
export function loadPrimeNative() {
  return (pending ||= (async () => {
    ...
    if (!cli?.packageDir) throw new HttpError(503, 'Installez Prime Agent pour gérer les MCP.');
    const localImport = (path) => import(pathToFileURL(join(cli.packageDir, path)).href);
    const [settings, auth, mcp, oauth] = await Promise.all([
    ...
    return { ...settings, ...auth, ...mcp, ...oauth, cli };
  })().catch(() => {
    pending = undefined;
    throw new HttpError(503, 'La gestion MCP nécessite une version compatible de Prime Agent.');
  }));
}
