...
/** Authenticated LAN gateway; the agent server remains loopback-only. */
export function createLanGateway({ host, upstreamPort, config }) {
  if (!isPrivateIPv4(host) && host !== '127.0.0.1')
  ...
  const gateway = createServer(async (req, res) => {
    ...
    try {
      ...
      if (peer !== '127.0.0.1' && !isPrivateIPv4(peer))
        return respond(res, 403, { error: 'Accès limité au réseau local.' });
      const now = Date.now();
      ...
      const writeRoute =
        ...
        (req.method === 'POST' && /^\/api\/live\/sessions\/[A-Za-z0-9_-]+\/(messages|queue)$/.test(url.pathname)) ||
      ...
      if (!reading) {
        ...
        for await (const chunk of req) {
          length += chunk.length;
          if (length > 512 * 1024) return respond(res, 413, { error: 'La demande dépasse 512 Ko.' });
          chunks.push(chunk);
        }
