...
const page = (
  error = '',
  accessLabel = 'Accès privé au Studio',
...
export function createLanGateway({ host, upstreamPort, config, publicOrigin }) {
  ...
  if (publicOrigin !== undefined) {
    ...
    if (host !== '127.0.0.1') throw new Error('La passerelle HTTPS doit écouter uniquement sur loopback.');
  }
  ...
  function login(res, status, error = '') {
    ...
    res.end(
      page(
        error,
        publicOrigin
          ? 'Accès privé · Tailscale · HTTPS'
          : isTailscaleIPv4(host)
            ? 'Accès privé · Tailscale'
            : 'Accès privé · réseau local',
      )
        .replace('</head>', PWA_LOGIN_HEAD + '</head>')
        .replace(
          '</main>',
          '<button id="pwa-install" class="pwa-install" type="button">Installer le Studio</button></main>',
        ),
    );
  }
  const gateway = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'POST' && url.pathname === '/lan/login') {
        ...
        res.writeHead(303, {
          Location: '/',
          'Set-Cookie': `${cookieName}=${token}; HttpOnly; SameSite=Strict; Path=/; Max-Age=28800${publicOrigin ? '; Secure' : ''}`,
        });
        res.end();
        return;
      }
      ...
      if (!publicResource && (!token || !sessions.has(token)))
        return respond(res, 401, { error: 'Saisissez le nouveau code d’accès sur la page d’accueil.' });
      const proxy = request(
        {
          hostname: '127.0.0.1',
          port: upstreamPort,
