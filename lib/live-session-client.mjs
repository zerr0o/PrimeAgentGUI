...
const unavailable = () => new LiveSessionError(409, 'Cette session n’est plus disponible en direct.');
...
function validateTarget(sessionId, cwd) {
  if (
    ...
    throw new LiveSessionError(400, 'La session ou le dossier du projet est invalide.', 'invalid_target');
  }
}
...
function validateText(text, { blank = false } = {}) {
  if (
    ...
    throw new LiveSessionError(400, 'Le message est vide ou dépasse 512 Ko.', 'invalid_message');
  }
  return text;
}
...
export function createLiveSessionClient(options, dependencies = {}) {
  ...
  if (
    ...
    throw new LiveSessionError(400, 'Le point d’accès au moteur est invalide.', 'invalid_endpoint');
  }
  ...
  async function request(client, command, { mutation = false } = {}) {
    ...
    try {
      ...
      throw mutation
        ? new LiveSessionError(
            503,
            'Le moteur n’a pas confirmé cette action. Vérifiez la file avant de réessayer.',
            'delivery_uncertain',
          )
        : unavailable();
    }
    if (!response?.success)
      throw new LiveSessionError(
        409,
        'Le moteur a refusé cette action sur la session active.',
        'native_rejected',
      );
  ...
  return {
    ...
    async send(sessionId, cwd, input = {}) {
      ...
      if (!['steer', 'follow_up'].includes(input.mode))
        throw new LiveSessionError(400, 'Le mode d’envoi est invalide.', 'invalid_mode');
      ...
      return withSession(sessionId, cwd, async (client, owner, state) => {
        if (images.length && Array.isArray(state.model?.input) && !state.model.input.includes('image'))
          throw new LiveSessionError(
            400,
            'Ce modèle ne prend pas en charge les images.',
            'unsupported_images',
          );
        ...
        if (sessionCommand && (images.length || /[\r\n\u2028\u2029]/.test(message)))
          throw new LiveSessionError(
            400,
            'Une commande de session s’écrit sur une seule ligne, sans image.',
            'invalid_command',
          );
    ...
    async mutate(sessionId, cwd, input = {}) {
      if (
        ...
        throw new LiveSessionError(400, 'La position dans la file est invalide.', 'invalid_queue_position');
      ...
      if (supplied?.type === 'delete') mutation = { type: 'delete' };
        ...
        throw new LiveSessionError(400, 'La modification de la file est invalide.', 'invalid_queue_mutation');
      return withSession(sessionId, cwd, async (client, owner) => {
        ...
        if (!['applied', 'rejected', 'invalid'].includes(data?.status))
          throw new LiveSessionError(
            502,
            'Le moteur a renvoyé une réponse de file invalide.',
