...
Les captures sont enregistrées dans `docs/screenshots/`. Trois vues du bureau sont capturées en 1600 × 1000 ; le sélecteur est cadré sur sa fenêtre pour rester lisible dans le README. Le rapport se trouve dans `test-results/readme-captures.json`. `PRIME_STUDIO_BROWSER` permet de choisir un autre canal Playwright installé, par exemple `chrome`.
